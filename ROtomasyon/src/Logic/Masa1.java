 ...
 * @author EMRE
 */
public class Masa1 implements ActionListener{
...
//            BMasa1.setVerticalTextPosition(SwingConstants.CENTER);
    ...
    public Masa1(){
        getMasa1Frame().add(getMasa1Panel());
        getMasa1Frame().setVisible(true);
        
         getMasa1Panel().add(getArayemekler());
        getMasa1Panel().add(getArayemekl());
        getMasa1Panel().add(getArayemeklr());        
        getMasa1Panel().add(getArayemekName());
        
        
        getMasa1Panel().add(getAnayemekler());
        getMasa1Panel().add(getAnayemekl());
        getMasa1Panel().add(getAnayemeklr());
        getMasa1Panel().add(getAnayemekName());
        
        getMasa1Panel().add(getSalata());       
        getMasa1Panel().add(getSalatal());
        getMasa1Panel().add(getSalatalr());        
        getMasa1Panel().add(getSalataName());

        getMasa1Panel().add(getIcecek());       
        getMasa1Panel().add(getIcecekl());
        getMasa1Panel().add(getIceceklr());
        getMasa1Panel().add(getIcecekName());

        getMasa1Panel().add(getsiparisGoruntule());
        getMasa1Panel().add(getgeriDon());
        getMasa1Panel().add(getanasayfaDon());
        getMasa1Panel().add(getekle());
        getMasa1Panel().add(getHesapal());
        getMasa1Panel().add(getTplmucret());  
...
public JFrame getMasa1Frame() {
        if (Masa1Frame == null) {
            Masa1Frame = new JFrame("MASA 1");
            Masa1Frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            Masa1Frame.setBounds(300, 10, 735, 720);
            
        }
        return Masa1Frame;
    }

    public void setMasa1Frame(JFrame Masa1Frame) {
        this.Masa1Frame = Masa1Frame;
    }
    
    public JPanel getMasa1Panel() {
        if (Masa1Panel == null) {
            Masa1Panel = new JPanel();
            Masa1Panel.setLayout(null);
            Masa1Panel.setBackground(Color.white);
            Masa1Panel.setBounds(0, 0, 0, 0);
    ...
    public JButton getsiparisGoruntule() {
        ...
        st=con.createStatement();
            ...
            rs=st.executeQuery("SELECT * FROM siparisler WHERE masano='1'");
    ...
    public void setSiparisGoruntule(JButton siparisGoruntule) {
        ...
        st=con.createStatement();
            ...
            String sorgu=String.format("DELETE FROM siparisler WHERE masano='1'");
            Statement stmt=con.createStatement(); 
            stmt.executeUpdate(sorgu);
            try{
            Statement stmtt=con.createStatement(); 
            String sorguu=String.format("insert into istatistik values(NULL, '1', %d)", fatura);
